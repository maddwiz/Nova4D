...
- UI responsibilities are split into browser modules under `server/ui/modules/` (`bridge_status`, `provider_management`, `workflow_engine`, `execution_engine`, `cinematic_smoke`, `run_monitor`, `command_browser`, `voice_control`, `studio_bootstrap`) with `app.js` as the orchestrator
