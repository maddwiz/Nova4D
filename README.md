...
- Retry failed cinematic-smoke stages and export a full cinematic-smoke JSON report from Studio
...
- Use voice shortcuts (`nova command smart run ...`, `nova command plan ...`, `nova command run ...`, `nova command run template`, `nova command run cinematic smoke`, `nova command retry smoke failures`, `nova command export smoke report`, `nova command load smoke history 2`, `nova command export smoke history 1`, `nova command clear smoke history`, `nova command guided check`, `nova command load last history`, `nova command load history 2`, `nova command export history 3`, `nova command clear history`)
...
- Use `npm run version:check` before release to catch package/docs/script version drift.
- Use `npm run legal:check` (or `bash scripts/check_legal_signoff.sh --strict`) before marketplace upload.
